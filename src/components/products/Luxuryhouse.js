...
import '../Products.css'
import Luxuryhousepic from './Images/Luxuryhouse.jpg'


class Luxuryhouse extends React.Component{
	...
	Luxuryhousebuy = (rate,items,name,trate) =>{
		this.props.moneyminus(rate,items,name,trate)
		 
	}
	Luxuryhousesell = (rate,items,name,trate) =>{
		this.props.moneyminus(-rate,-items,name,trate)
		...
		if(this.state.value>=1){
			 sellbutton= <div className='ui button sell' style={{backgroundColor:'red',color:'white'}} onClick={
						()=>{						
							...
							const name = 'Luxuryhouse'
							this.Luxuryhousesell(rate,items,name,trate)
		...
		if(this.state.rate<this.props.Money){
			buybutton=	<div className='ui button buy' style={{backgroundColor:'#248f24',color:'white'}} onClick={
						()=>{
							...
							const items = this.state.items
							const name = 'Luxuryhouse'
							this.Luxuryhousebuy(rate,items,name,trate)
		...
		return(
			//color:'rgb(255, 0, 93)',fontSize:'20px',fontFamily:'robotic',marginBottom:'7px'
				<div  className='container-2' >
					<div className='Head' style={{fontSize:'25px'}} >Luxury House</div>
					...
					<center>
					<img src={Luxuryhousepic} loading='lazy' className='img' alt='not found' / ></center><p style={{fontSize:'20px',position:'absolute',right:'10px',bottom:'-25px'}}>x{this.state.value}</p>
					</div>
					<br/><br/>
